...
OmniMerchant v1 supported a Custom App token path. v4 keeps the Shopify connector as a production-oriented backbone: OAuth install, HMAC/state validation, sync jobs, webhook status processing, duplicate detection, privacy webhook handling, bulk-operation tracking, and replay APIs.
