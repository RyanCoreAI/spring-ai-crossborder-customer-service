      ...
      <a-form ref="formRef" :model="form" :rules="rules" layout="vertical">
          ...
          <a-col :span="24">
            <a-form-item label="文档类型" name="docType">
              <a-select v-model:value="form.docType">
                <a-select-option value="REFUND_POLICY"
                  >退款政策</a-select-option
                >
                <a-select-option value="SHIPPING_POLICY"
                  >物流政策</a-select-option
                >
                <a-select-option value="FAQ">FAQ</a-select-option>
                <a-select-option value="PRODUCT_GUIDE"
                  >产品指南</a-select-option
                >
                <a-select-option value="PRIVACY_POLICY"
                  >隐私政策</a-select-option
                >
                <a-select-option value="TERMS_OF_SERVICE"
                  >服务条款</a-select-option
                >
              </a-select>
            </a-form-item>
          </a-col>
        ...
        <a-form-item label="内容" name="rawContent">
          <a-textarea
            ...
            :rows="8"
            placeholder="粘贴政策文档、FAQ 或产品指南内容"
          />
        </a-form-item>
...
import type { KnowledgeDoc } from "@/types/contracts";
...
async function loadData() {
  loading.value = true;
  try {
    const params: { page: number; size: number; tenantId?: number; docType?: string } = { page: page.value, size: size.value };
...
function resetForm() {
  Object.assign(form, {
    tenantId: getStoredTenantId() ? Number(getStoredTenantId()) : null,
    docType: "FAQ",
    title: "",
    summary: "",
    language: "zh",
...
function showDialog(row?: KnowledgeDoc) {
