  ...
  <div class="trust-console">
    <div class="page-head console-head">
      <a-space class="head-actions" :size="10">
        ...
        <a-button type="primary" :loading="runningEval" @click="runEval"
          >运行评测</a-button
        >
      </a-space>
    </div>
      ...
      <a-card class="health-card">
        <template #title>
          <div class="section-title">
            <span>核心健康信号</span>
            <a-tag :color="overallHealth.color">{{
              overallHealth.label
            }}</a-tag>
          </div>
        </template>
        ...
        <div class="signal-list">
          ...
          >
            ...
            <div class="signal-bar" :class="{ empty: signal.value === null }">
              <span
                v-if="signal.value !== null"
                :style="{ width: `${signal.value}%` }"
              ></span>
            </div>
            <div
              class="signal-value"
              :class="{ warn: signal.value !== null && signal.value < 95 }"
            >
    ...
    <ObservabilityEvidenceTables :tools="toolMetrics" :trend="evalTrend" />
  </div>
</template>
...
import api from "@/api";
import ObservabilityEvidenceTables from "@/components/admin/observability/ObservabilityEvidenceTables.vue";
import type {
  EvalTrendPoint,
  FailureBucket,
  ObservabilitySummary,
  RagObservabilityMetric,
  ToolMetric,
} from "@/types/observability";
import type { TraceSummary } from "@/types/contracts";

const router = useRouter();
const loading = ref(false);
const runningEval = ref(false);
const summary = ref<ObservabilitySummary | null>(null);
const ragMetrics = ref<RagObservabilityMetric | null>(null);
const traces = ref<TraceSummary[]>([]);
const failures = ref<FailureBucket[]>([]);
const toolMetrics = ref<ToolMetric[]>([]);
const evalTrend = ref<EvalTrendPoint[]>([]);

const topMetrics = computed(() => [
  {
    label: "评测通过率",
    value: hasPositiveNumber(summary.value?.evalRuns)
      ? formatPercent(summary.value?.latestEvalPassRate)
      : "—",
    note: formatCount(summary.value?.evalRuns, "次评测运行"),
  },
  {
    label: "工具成功率",
    value: hasPositiveNumber(summary.value?.toolCalls)
      ? formatPercent(summary.value?.toolSuccessRate)
      : "—",
    note: formatCount(summary.value?.toolCalls, "次工具调用"),
  },
  {
    label: "RAG 引用覆盖率",
    value: hasPositiveNumber(ragMetrics.value?.evalRuns)
      ? formatPercent(ragMetrics.value?.citationCoverage)
      : "—",
  ...
  },
  {
    label: "RAG 检索质量",
    value: hasPositiveNumber(ragMetrics.value?.evalRuns)
      ? formatPercent(ragMetrics.value?.recallAtK)
      : "—",
    note: hasPositiveNumber(ragMetrics.value?.evalRuns)
  ...
  {
    label: "单解决会话成本",
    value: hasPositiveNumber(summary.value?.aiResolved)
      ? formatUsd(summary.value?.costPerResolvedConversation)
      : "—",
...
const healthSignals = computed(() => [
  {
    ...
    value: hasPositiveNumber(summary.value?.evalRuns)
      ? percentNumber(summary.value?.latestEvalPassRate)
      : null,
    path: "/admin/evals",
  },
  {
    ...
    value: hasPositiveNumber(ragMetrics.value?.evalRuns)
      ? percentNumber(ragMetrics.value?.poisoningBlockRate)
      : null,
  ...
  },
  {
    ...
    value: hasPositiveNumber(ragMetrics.value?.evalRuns)
      ? percentNumber(ragMetrics.value?.noAnswerAccuracy)
      : null,
...
const overallHealth = computed(() => {
  const values = healthSignals.value
    .map((item) => item.value)
...
const focusItems = computed(() => {
  ...
  if (Number.isFinite(failedToolCalls) && failedToolCalls > 0) {
    items.push({
      ...
      description: summary.value?.topFailedTool
        ? `最常失败工具：${summary.value.topFailedTool}`
        : "后端未返回最常失败工具。",
    });
  }
  if (Number.isFinite(failedTraces) && failedTraces > 0) {
    items.push({
      title: `${failedTraces} 条失败轨迹`,
      description: "可进入轨迹回放查看失败分类。",
    });
  }
  if (Number.isFinite(safetyBlocks) && safetyBlocks > 0) {
    items.push({
      title: `${safetyBlocks} 次安全拦截`,
      description: "来自后端 safetyBlocks 计数。",
    });
  }
  if (Number.isFinite(webhookBacklog) && webhookBacklog > 0) {
    items.push({
      title: `${webhookBacklog} 个 Shopify Webhook 积压`,
      description: "可在渠道集成页处理重试或重放。",
    });
  }
  if (Number.isFinite(fallbackRate) && fallbackRate > 0) {
    items.push({
      title: `Fallback 率 ${formatPercent(fallbackRate)}`,
      description: "后端观测到降级响应。",
    });
  }
...
function traceMeta(trace: TraceSummary & { modelName?: string; toolCallCount?: number }) {
  const parts = [];
  if (trace.modelName) parts.push(trace.modelName);
  if (hasValue(trace.totalLatencyMs)) parts.push(`${trace.totalLatencyMs}ms`);
  if (hasValue(trace.toolCallCount))
    parts.push(`${trace.toolCallCount} 次工具`);
  return parts.length ? parts.join(" · ") : "后端未返回摘要";
}

function goSignal(signal: { path: string }) {
  router.push(signal.path);
}

function openTrace(trace: { traceId?: string }) {
  if (trace.traceId) {
...
async function load() {
  loading.value = true;
  try {
    const [summaryRes, failureRes, traceRes, ragRes, toolRes, trendRes] =
      await Promise.all([
