        ...
        <a-row :gutter="16">
          <a-col :span="12">
            <a-form-item label="平台" name="platform">
              <a-select v-model:value="form.platform">
                ...
                <a-select-option value="woocommerce"
                  >WooCommerce</a-select-option
                >
                <a-select-option value="tiktok_shop"
                  >TikTok Shop</a-select-option
                >
          ...
          <a-col :span="12">
            <a-form-item label="平台店铺 ID" name="externalStoreId">
              <a-input
                v-model:value="form.externalStoreId"
                placeholder="外部平台店铺标识"
              />
            </a-form-item>
          </a-col>
        </a-row>
...
import api from "@/api";
import type { TenantSummary } from "@/types/contracts";

const loading = ref(false);
const saving = ref(false);
const tableData = ref<TenantSummary[]>([]);
const page = ref(1);
const size = ref(20);
const total = ref(0);
...
const rules = {
  ...
  externalStoreId: [
    { required: true, message: "请输入平台店铺 ID", trigger: "blur" },
  ],
...
function showDialog(row?: TenantSummary) {
...
function handleDelete(row: TenantSummary) {
  Modal.confirm({
    title: "确认删除租户？",
