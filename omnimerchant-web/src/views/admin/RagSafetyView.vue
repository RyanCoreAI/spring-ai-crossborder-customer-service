      ...
      >
        <template #bodyCell="{ column, record }">
          ...
          <template v-else-if="column.key === 'indexAllowed'">
            <a-tag :color="record.indexAllowed ? 'green' : 'red'">{{
              record.indexAllowed ? "允许" : "禁止"
            }}</a-tag>
          </template>
          <template v-else-if="column.key === 'sourceTrustLevel'">
            <a-tag :color="trustColor(record.sourceTrustLevel)">{{
              trustLabel(record.sourceTrustLevel)
            }}</a-tag>
          </template>
...
import api from "@/api";
import type { RagSafetyReview } from "@/types/contracts";

const loading = ref(false);
const reviews = ref<RagSafetyReview[]>([]);
...
const columns = [
  ...
  {
    title: "来源可信度",
    dataIndex: "sourceTrustLevel",
    key: "sourceTrustLevel",
    width: 120,
  },
...
async function approve(row: RagSafetyReview) {
  await api.post(`/rag/safety/docs/${row.docUuid}/approve`, {
    note: "从管理后台审核通过",
  });
...
async function reject(row: RagSafetyReview) {
  await api.post(`/rag/safety/docs/${row.docUuid}/reject`, {
    note: "从管理后台拒绝索引",
  });
