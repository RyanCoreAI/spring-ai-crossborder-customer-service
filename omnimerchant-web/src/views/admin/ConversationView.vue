        ...
        <a-spin :spinning="loading">
          <div class="conv-items">
            <button
              ...
              class="conv-card"
              :class="{
                active: selectedUuid === conversation.conversationUuid,
              }"
              type="button"
      ...
      <a-card v-else class="message-panel">
        <template #title>
          <div class="message-title">
            ...
            <a-space>
              <a-tag
                >意图：{{ intentLabel(selectedConv?.intentPrimary) }}</a-tag
              >
              <a-tag color="gold"
                >情绪：{{ selectedConv?.sentiment || "未知" }}</a-tag
              >
            </a-space>
          </div>
        </template>
        ...
        <a-spin :spinning="msgLoading">
          <div class="message-list">
            ...
            <a-empty
              v-if="!messages.length && !msgLoading"
              description="暂无消息"
            />
          </div>
        </a-spin>
...
import { intentLabel } from "@/utils/display";
import type { ConversationMessage, ConversationSummary } from "@/types/contracts";
...
async function loadData() {
  loading.value = true;
  try {
    const params: { page: number; size: number; status?: number } = { page: page.value, size: size.value };
...
async function selectConversation(conversation: ConversationSummary) {
  ...
  try {
    const res = await api.get(
      `/conversations/${conversation.conversationUuid}/messages`,
    );
