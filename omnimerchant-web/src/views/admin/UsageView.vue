...
import api from "@/api";
import type { DashboardCommerceSummary, UsageSummary } from "@/types/contracts";
...
async function load() {
  try {
    ...
    const data = (dash.data || {}) as DashboardCommerceSummary;
    metrics.value = [
      {
        label: "AI 解决率",
        value: formatValue(data.aiResolutionRate),
        suffix: hasValue(data.aiResolutionRate) ? "%" : undefined,
      },
      {
        label: "人工升级率",
        value: formatValue(data.escalationRate),
        suffix: hasValue(data.escalationRate) ? "%" : undefined,
      },
      {
        label: "工具成功率",
        value: formatValue(data.toolSuccessRate),
        suffix: hasValue(data.toolSuccessRate) ? "%" : undefined,
      },
