    ...
    <a-drawer v-model:open="drawer" title="轨迹回放详情" width="64%">
      <a-descriptions v-if="detail.run" :column="2" bordered class="detail">
        ...
        <a-descriptions-item label="意图">{{
          intentLabel(detail.run.intent)
        }}</a-descriptions-item>
        <a-descriptions-item label="模型">{{
          detail.run.modelName || "-"
        }}</a-descriptions-item>
        <a-descriptions-item label="失败分类">{{
          failureCategoryLabel(detail.run.failureCategory)
        }}</a-descriptions-item>
        <a-descriptions-item label="总延迟">{{
          formatLatency(detail.run.totalLatencyMs)
        }}</a-descriptions-item>
      </a-descriptions>
      ...
      <a-timeline v-else class="timeline">
        <a-timeline-item v-for="step in detail.steps" :key="step.stepIndex">
          <div class="step-title">
            {{ step.stepIndex }}.
            {{ step.name || stepTypeLabel(step.stepType) }}
            <a-tag :color="statusColor(step.status)">{{
              statusLabel(step.status)
            }}</a-tag>
          </div>
          <div v-if="step.inputSummary" class="step-text">
            输入摘要：{{ step.inputSummary }}
          </div>
          <div v-if="step.outputSummary" class="step-text">
            输出摘要：{{ step.outputSummary }}
          </div>
          <div class="step-meta">
            {{ formatLatency(step.latencyMs) }}
            {{ failureCategoryLabel(step.failureCategory) }}
          </div>
        </a-timeline-item>
      </a-timeline>
    </a-drawer>
...
import type { TraceDetail, TraceSummary } from "@/types/contracts";

const route = useRoute();
const loading = ref(false);
const drawer = ref(false);
const traces = ref<TraceSummary[]>([]);
const detail = ref<TraceDetail>({});
...
function traceRow(record: TraceSummary) {
  return {
...
async function load() {
  loading.value = true;
  try {
    const res = await api.get("/observability/traces", {
      params: { ...filters, page: 1, size: 50 },
    });
...
async function openTrace(row: Pick<TraceSummary, "traceId">) {
...
onMounted(async () => {
  ...
  if (route.query.traceId) {
    await openTrace({ traceId: String(route.query.traceId) });
  }
});
</script>
